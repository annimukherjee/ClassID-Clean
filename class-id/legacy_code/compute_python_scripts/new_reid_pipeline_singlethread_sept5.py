    ...
    for SESSION_DIR in session_dirs:
        ...
        for SESSION_CAMERA in ['front']:
            ...
            for SESSION_CAMERA_FILE in SESSION_CAMERA_FILES:
                ...
                session_frames_output_dir = f'/mnt/ci-nas-cache/edulyzeV2/pose_face_gaze_emb_reid/{COURSE_ID}/{SESSION_KEYWORD}-{SESSION_CAMERA}'
                ...
                for frame_number, video_frame in enumerate(mmcv_video_frames):
                    if (num_skip_frames == 0) | (frame_number % num_skip_frames == 0):
                        ...
                        frame_output_file = f'{session_frames_output_dir}/{frame_number}.pb'
...
#                                 faces_all = face_result[0]
#                                 if len(faces_all) > 1:
#                                     Y_TL_face_results = [face[1] for face in faces_all] 
#                                     Y_TL_min_index = np.argmin(Y_TL_face_results)
#                                     faces_all = np.array([faces_all[Y_TL_min_index]])
#                                 max_face_per_body = max(max_face_per_body, len(faces_all))
...
#                         logger.info(
#                             f"Frame: {frame_number} | face | {time_diff(face_process_start, face_process_end):.3f} secs [{body_count},{max_face_per_body}]")
...
#                             f"Frame: {frame_number} | gaze | {time_diff(gaze_process_start, gaze_process_end):.3f} secs")
                        ...
                        # get reid feature vector output
                            ...
                            for body_index, tracking_info in enumerate(frame_results):
                                if type(tracking_info) == dict:
                                    ...
                                    if (X_TL<0) | (Y_TL<0) | (X_BR<0) | (Y_BR<0):
                                        ...
                                        logger.warning("Negative boundaries for bounding boxes, skipping reid extractor...")
                                        continue
...
#                                     except:
#                                         logger.error(f"Error for face for body {body_index}, frame: {frame_number}")
