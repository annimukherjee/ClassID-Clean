...
VIDEO_DIR = f'./video-store/{sys.argv[1]}/video_backup'
...
if __name__ == '__main__':
    ...
    for SESSION_DIR in session_dirs: # Outer Loop: Each class session (e.g., different dates)
        ...
        for SESSION_CAMERA in ['front']: # Each camera angle (currently only 'front')
            ...
            print(f"SESSION_CAMERA_FILES: {SESSION_CAMERA_FILES}")
            ...
            for SESSION_CAMERA_FILE in SESSION_CAMERA_FILES: # Each video file for that camera
                ...
                print(f"SESSION_CAMERA_FILE: {SESSION_CAMERA_FILE}")
                print(f"SESSION_KEYWORD: {SESSION_KEYWORD}")

                
                # get logger -----------------------------------------------------------------------
                session_log_dir = f'cache/tracking_singlethread_only/logs/{COURSE_ID}_MADENOW'
    ...
    #             # start loop with frames and video handler
                ...
                print(f"class_video_file: {class_video_file}")
                
                if not os.path.exists(class_video_file):
                    logger.info(f"Video File {class_video_file} not available, skipping session...")
                    continue
                
                print("pretending to read the video file now using mmcv.VideoReader()....")
                # mmcv_video_frames = mmcv.VideoReader(class_video_file)
                print("done pretending to read the video file now mmcv.VideoReader()....")
                ...
                print(f"num_skip_frames: {num_skip_frames}")



                session_process_start = datetime.now()
                print(f"Session processing started at: {session_process_start}")
