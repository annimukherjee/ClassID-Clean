...
session_filter_list = ['classinsight-cmu_05681A_ghc_4301_201905011630', 'classinsight-cmu_05410B_ghc_4211_201901281500'
]
...
TARGET_FPS = 5
# FRAME_INTERVAL_IN_SEC = 0.5           # useless.
...
facial_embedding_model = InceptionResnetV1(pretrained=session_config['face_embedding_model_name'], device=DEVICE).eval()
...
if __name__ == '__main__':
    for SESSION_DIR in session_dirs: # for each session:
        SESSION_KEYWORD = SESSION_DIR.split("/")[-1]
        
        
        for SESSION_CAMERA in ['front']: # there's only one camera for now.
            SESSION_CAMERA_FILES = glob.glob(f'{SESSION_DIR}/*{SESSION_CAMERA}.avi')


            for SESSION_CAMERA_FILE in SESSION_CAMERA_FILES: # for each camera file:
                SESSION_KEYWORD = \
                ...
                if not os.path.exists(class_video_file):
                    ...
                    continue
                mmcv_video_frames = mmcv.VideoReader(class_video_file) # read the video file
